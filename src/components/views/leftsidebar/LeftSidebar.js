...
export default class LeftSidebar extends React.Component {
    ...
    render() {
        ...
        if (leftSideBarComponent.top_logo.format == "text") {
            logoWrapper = (
                <h2 className="app-title">{leftSideBarComponent.top_logo.text_config.text}<sub
                    className="app-version">{leftSideBarComponent.top_logo.text_config.release_version}</sub>
            ...
            logoWrapper = (
                <div >
                    <img className="app-logo" width={leftSideBarComponent.top_logo.img_config.width}
                         src={leftSideBarComponent.top_logo.img_config.source}/>
                    <hr className="title-hr"/>
                </div>
            );
        }
            ...
            <div>
                ...
                <div className="row-fluid leftsidebar-bottom-logos">
                    {leftSideBarComponent.bottom_logo.contributor_organizations.map(function (image) {
                        return (<a href={image.link} target="_blank" className="organization-logo-link"
